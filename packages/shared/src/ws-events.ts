...
export interface ServerToClientEvents {
  'job:status': (p: { jobId: string; status: PrintJobStatus; eta?: number }) => void;
  'job:progress': (p: { jobId: string; pagesPrinted: number; pagesTotal: number }) => void;
  'printer:status': (p: {
    printerId: string;
...
export interface ClientToServerEvents {
  'agent:heartbeat': (p: {
    agentId: string;
    shopId: string;
    printers: { id: string; status: PrinterStatus }[];
  }) => void;
  'agent:job-claimed': (p: { jobId: string; agentId: string }) => void;
  'agent:job-result': (p: {
    jobId: string;
